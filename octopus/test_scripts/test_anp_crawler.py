...
class ANPCrawlerTest:
    ...
    async def wait_for_server(self, host: str = "localhost", port: int = None, max_wait: int = 30) -> bool:
        """
    ...
    def get_did_paths(self) -> tuple[str, str]:
        ...
        did_document_path = (
            self.settings.did_document_path or
            str(Path(__file__).parent.parent.parent / "docs/user_public/did.json")
        )
        private_key_path = (
            self.settings.did_private_key_path or
            str(Path(__file__).parent.parent.parent / "docs/user_public/key-1_private.pem")
        )
    ...
    async def test_local_ad_json_access(self) -> bool:
        ...
        try:
            ...
            if result.get("type") == "error":
                self.logger.error(f"❌ ANP Crawler test failed: {result.get('content')}")
                return False
                ...
                for doc in crawled_documents:
                    if doc.get("url") == local_url:
                        ...
                        if isinstance(content, dict) and content.get("name"):
                            self.logger.info(f"✅ Retrieved valid agent description: {content.get('name')}")
                            return True
                        break
