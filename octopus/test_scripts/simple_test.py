                ...
                if content:
                    ...
                    if isinstance(data, dict):
                        ...
                        if 'interfaces' in data:
                            ...
                            if data['interfaces'] and 'content' in data['interfaces'][0]:
...
def main():
    ...
    rpc_tests = [
        ("message.send_message", {"message_content": "测试消息", "recipient_did": "did:example:test"}, "both", "success"),
        ("message.get_statistics", {}, "both", "success"),
        ("message.get_message_history", {"other_did": "did:example:test", "limit": 10}, "external", "success"),
        ("message.receive_message", {"message_content": "内部消息", "sender_did": "did:example:sender"}, "internal", "error"),
    ...
    for method, params, access_level, expected in rpc_tests:
        ...
        if (expected == "success" and result == "success") or (expected == "error" and result == "error"):
            print("✅ 测试符合预期")
            rpc_success += 1
        else:
    ...
    print(f"总体通过率: {(success_count + rpc_success)}/{(total_tests + rpc_total)} ({((success_count + rpc_success)/(total_tests + rpc_total)*100):.1f}%)")
